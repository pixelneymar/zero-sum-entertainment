# Accordion — TypeUI · Cypherpunk

> **TypeUI · Cypherpunk** — stacked expand/collapse panels.
> Depends on: `colors.md`, `radius.md`, `shadows.md`, `typography.md`

An accordion folds long content — FAQs, settings sections, help — into scannable rows. In Cypherpunk it comes in three builds (grouped, separated cards, flush), but the silhouette is constant: **tight** (`radius-xxl`, 2px) on the outer shell, **no perimeter border**, 2px `default` dividers between items (functional row separators), and a chevron that rotates 180° as a panel opens. Triggers stay quiet (`body` closed, `heading` on a soft fill when open); only the brand-emphasis variant raises the volume. Depth is flat throughout — separation comes from a lighter card surface and the row dividers, never a shadow.
...
| Body text | `body` |
| Top edge | 2px `default` border separating panel from trigger above |
...
|---|---|---|
| Root | None — lighter card surface defines the group | `radius-xxl` (2px) on outer corners |
| Root overflow | Clip children to root radius | — |
| Item separator | 2px `default` bottom border on each item except the last | — |
...
| Item separator | 2px `default` bottom border only | None | Transparent triggers and panels |
...
- **No raw hex or palette steps**, and no one-off border widths — dividers are 2px `default` unless a variant table says otherwise.
