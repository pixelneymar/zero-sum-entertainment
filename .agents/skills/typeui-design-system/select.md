...
The minimal variant: no side border, **bottom border only** (2px `default-medium`), transparent or `neutral-primary` background. Focus turns the bottom border `brand` with an optional inward 4px `brand-medium` ring. Radius is `radius-none` on this shell — the only case where a select shows no boxed shell at all — just a bottom rule.
