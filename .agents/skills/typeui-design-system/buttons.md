# Buttons — TypeUI · Cypherpunk

> **TypeUI · Cypherpunk** — the action layer of the system.
> Depends on: `colors.md`, `radius.md`, `shadows.md`, `spacing.md`, `typography.md`, `badges.md`

In Cypherpunk, a button is a crisp, **near-square** (`radius-xxl`, 2px) control with a solid fill, **no shadow**, and a brand focus ring — classic and rectangular, like the buttons on the reference site. Buttons are confident but never loud: exactly one filled **brand** primary leads each section, and everything else steps back to secondary, tertiary, ghost, outline, or link. The saturated **ink brand** carries "the next step" and always pairs with a **`white` label** (never dark text on the ink fill); the status fills (success, danger, warning) appear only when the action genuinely is that.
...
| Radius | `radius-xxl` (2px) — every size and variant |
...
| **Secondary** | `neutral-primary-soft` (`#EAE5DB`) | `body` | `default-medium` | `neutral-tertiary-medium` + `heading` text | `neutral-tertiary` |
...
The defining button of Cypherpunk is a **classic, solid, near-square control** — exactly like the buttons on the reference site. The primary rests on a solid `brand` (`#1C1C1C`) fill with a `white` label and **near-square 2px corners**; on hover the fill deepens one step to `brand-strong`; on press it settles darker still. No sweep, no wedge, no animated frame — just a confident rectangle that darkens on interaction. The rule is **stack-agnostic**: it describes which tokens supply each value, so it can be built with plain CSS, a CSS-in-JS layer, a utility framework, or any renderer.
...
| Resting label | `white` (the ink fill demands a light label) |
...
| Corner radius | `radius-xxl` (2px) — `radius.md` |
...
button {
  ...
  font-family: inherit;            /* → font-family (Untitled Sans) */
  font-weight: 500;                /* → font-weight-medium */
  line-height: 1.3;                /* → line-height-component */
  color: var(--white);             /* label on the ink fill */
  background: var(--brand);        /* #1C1C1C */
  border: none;
...
A near-square control — width equals height per tier — for toolbars and compact actions. Use any filled, outline, or ghost row above.
...
Not part of core Cypherpunk. Default product UI is solid fills only. If a campaign needs a gradient, define the paired tokens in `colors.md` and `shadows.md` first — do not hand-roll them on the button.

---

## Motion

Cypherpunk buttons respond instantly and quietly — no bounce, no theatrics.
...
- **No corners other than `radius-xxl`** (2px) — rounded or pill button corners are a different theme, not Cypherpunk.
...
- **No two primary brand buttons** side by side in one action group — Cypherpunk allows a single obvious next step.
...
- **Prohibited:** an outline (or any bordered) button that renders taller than the solid button of the same size. Height parity across variants is mandatory.
