# Button Group — TypeUI · Cypherpunk

> **TypeUI · Cypherpunk** — related actions fused into one control.
> Depends on: `colors.md`, `radius.md`, `shadows.md`, `spacing.md`, `typography.md`, `buttons.md`, `dropdowns.md`, `badges.md`

A button group reads as a single object: segments butt together, separated only by a 2px `default` divider on the shared edge, and only the outer corners keep the Cypherpunk **tight** (`radius-xxl`, 2px). The group is flat and borderless (`elevation-none`) — no outer outline, no shadow. Use a group only for genuinely related choices — toolbars, segmented controls, paired actions — never as a row of unrelated buttons.
...
| **Divider** | Shared 2px border between segments (not doubled) |
...
The fusion is the whole point: segments overlap by 2px so adjacent borders become one line, and rounding lives only on the group's outer corners.

| Property | Token / value |
|---|---|
| Direction | Horizontal (default) or vertical |
| Shadow | `elevation-none` on root (not per segment) |
| Segment overlap | Negative inline margin 2px so borders merge |
...
- **No shadow on the group or its segments** — the group is flat (`elevation-none`); separation is the shared 2px divider only.
