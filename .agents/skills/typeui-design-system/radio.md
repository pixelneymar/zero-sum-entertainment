...
| Border | `default` (`#1C1C1C`), 2px (unchecked); `brand` when selected |
| Radius | `radius-full` |
...
A row card: padding `spacing-4`, **`radius-xxl`**, background matching the surface (`#EAE5DB`) with a `default` (`#1C1C1C`) border. Selected is marked by a `brand` border (not a fill change — the background still matches the surface).

### Radio list group

A vertical grouped list matching the surface (`#EAE5DB`) with a `default` (`#1C1C1C`) border around the group (square **`radius-xxl`** corners) and 2px `default` row dividers. The section heading above sits at `font-size-base`, `font-weight-semibold`, `heading`, `spacing-3` below.
