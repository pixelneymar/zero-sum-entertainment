...
| Part | Token / value |
|---|---|
| Outer wrapper | `default` (`#1C1C1C`) border, `radius-xxl`, background matching the surface (`#EAE5DB`), flat (`elevation-none`) |
| Toolbar | Row between top border and text; padding `spacing-2` `spacing-3`; bottom border 2px `default-medium` |
...
| Part | Token / value |
|---|---|
| Wrapper | Same bordered (`default`), flat, near-square card as WYSIWYG, without the top toolbar |
...
| Text control | Flex-grow; single-line height with `rows="1"`; shell matching the surface (`#EAE5DB`) with a `default` border, `radius-xxl`, padding `spacing-2-5` `spacing-3` |
