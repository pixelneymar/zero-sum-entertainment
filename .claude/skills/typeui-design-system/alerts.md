...
Block alerts and the inline announcement both wear the signature **tight** (`radius-xxl`, 2px) — nothing rounds.

| Variant | Border | Radius |
|---|---|---|
| **Default** | None | `radius-xxl` |
| **Bordered** | 2px intent border on all sides | `radius-xxl` |
...
| **Announcement** | 2px intent border on all sides | `radius-xxl` (2px) |
...
Default or icon layout plus a **2px** intent border on the full perimeter — use it when the alert sits on a surface that matches its background and needs a defined edge.
...
| Badge | Intent medium/soft fill; `font-size-sm`; `font-weight-medium`; `radius-xxl` (2px); padding `spacing-0-5` vertical, `spacing-2` horizontal |
| Message gap after badge | `spacing-2` |
| Trailing chevron | 16 × 16px; `spacing-1` margin inline-start; inherits foreground |
| Border | 2px intent border |
...
- **No off-scale radius** — every alert, block or announcement, is square (`radius-xxl`, 2px); nothing rounds.
