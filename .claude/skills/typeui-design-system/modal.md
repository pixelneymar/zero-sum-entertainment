# Modal — TypeUI · Cypherpunk

> **TypeUI · Cypherpunk** — focused dialogs over a dimmed page.
> Depends on: `colors.md`, `radius.md`, `shadows.md`, `spacing.md`, `typography.md`, `buttons.md`, `input-field.md`

A modal interrupts the flow for a confirmation, a form, or critical information — used sparingly, since most tasks belong inline or in a drawer. The Cypherpunk panel is a clean `neutral-primary-soft` surface with the signature **tight** (`radius-xxl`, 2px) corners — **borderless**, lifted on a soft ink shadow (`elevation-4`) and reading clearly as a lighter panel above a 50% scrim. Structure is consistent: a header with title + close divided by a 2px functional rule, a scrolling body, and a footer (same 2px divider) that holds one primary action beside a secondary.
...
| Panel shadow | Soft ink (`elevation-4`) — plus the scrim for separation |
...
| Header border bottom | 2px `default` |
...
- **No panel border** — the panel is borderless; it lifts on `elevation-4` above the backdrop scrim and its lighter surface, never below `radius-xxl`.
