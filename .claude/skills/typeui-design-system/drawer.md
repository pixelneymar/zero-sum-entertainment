# Drawer — TypeUI · Cypherpunk

> **TypeUI · Cypherpunk** — off-canvas panels for navigation, forms, and detail sheets.
> Depends on: `colors.md`, `radius.md`, `shadows.md`, `spacing.md`, `typography.md`, `buttons.md`

A drawer slides in from a viewport edge — unlike a modal, it's anchored to that edge and often keeps wayfinding context in view. It shares the modal's `neutral-primary-soft` lighter-panel surface and is **borderless**, lifted on a soft ink shadow (`elevation-4`) — separated from the page by that shadow, its lighter surface, and the backdrop scrim — but its corners follow the **edge-anchored exception** to the square (0px) rule: edges flush against the viewport stay square, and only a free inward edge may round to `radius-xxl`. A full-height side drawer is therefore square by design.
...
| Panel radius | **Square on edges flush to the viewport**; the inward free edge may take `radius-xxl` (2px). A full-height side drawer stays square — see the edge-anchored exception in `radius.md` |
| Panel shadow | Soft ink (`elevation-4`) |
...
| Header bottom border | 2px `default`; padding bottom `spacing-4`; margin bottom `spacing-5` |
...
| Mode | Scrim |
|---|---|
| **Visible** | Semi-transparent overlay `rgba(0,0,0,0.5)` or `dark-strong` at documented opacity — tap closes |
...
- **Drawer shadow is `elevation-4`** — separation is that soft ink shadow plus the backdrop scrim and its lighter panel surface.
