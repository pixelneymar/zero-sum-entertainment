...
|---|---|
| Width | 100% of parent |
| Background | Raised `neutral-primary-soft` (`#EAE5DB`) surface |
| Border | `default` (`#1C1C1C`), 2px — the resting edge that defines the field |
...
A horizontal row where the addon segment and control share one outer radius — the fusion reads as a single near-square control.

| Part | Token / value |
|---|---|
| Row shadow | None (`elevation-none`) |
| Row border | `default` (`#1C1C1C`) on the outer perimeter |
| Row radius | `radius-xxl` on outer corners only |
| Addon background | Same as the field (matches the surface) |
| Addon border | A `default` (`#1C1C1C`) divider on the shared edge separates addon from control |
...
- **No divergent field shell** — same border, near-square (2px) radius, and elevation as the standard field shell.
...
|---|---|---|---|---|
| Default | Raised `#EAE5DB` surface | `default` (`#1C1C1C`) | `heading` / `body` | `brand-medium` |
...
- **No contrasting resting fill** — the field background matches its surface; the `default` (`#1C1C1C`) border defines the shell, and focus swaps it to a `brand` edge.
