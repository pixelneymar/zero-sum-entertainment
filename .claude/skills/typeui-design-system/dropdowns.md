...
Menu items are square (`radius-md`, 2px) — concentric with the panel, like every shell in this 0px system (see `radius.md`).
...
2px `default-medium`; vertical margin `spacing-2`.
...
- **No panel border** — it's a borderless lighter panel lifted on a soft ink shadow (`elevation-2`) floating over the section; items keep `radius-md` (concentric), nothing else.
