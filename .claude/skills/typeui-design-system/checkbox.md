# Checkbox — TypeUI · Cypherpunk

> **TypeUI · Cypherpunk** — multi-select boolean controls.
> Depends on: `colors.md`, `radius.md`, `shadows.md`, `spacing.md`, `typography.md`, `dropdowns.md`

A checkbox lets users pick any number of independent options. The control is a small 16px box that fills `brand` with a `white` check when selected (a light check, because the ink brand needs light marks for contrast). It is squared off like every other shell in Cypherpunk: its box stays at `radius-xs` (0px) so it reads as a crisp, near-square, never a radio — the square shell rule applies to the cards and list groups it sits inside as well as the tick box itself. When wrapped in a bordered card or list group, that container takes the signature **tight** (`radius-xxl`, 2px).
...
|---|---|
| Size | 16 × 16px |
| Background (unchecked) | Raised `#EAE5DB` surface |
| Background (checked) | `brand` with check mark `white` (light, for contrast on ink) |
| Border | `default` (`#1C1C1C`), 2px (unchecked); `brand` when checked |
| Radius | `radius-xs` |
...
The whole row wrapped in a card: padding `spacing-4`, **`radius-xxl`**, background matching the surface (`#EAE5DB`) with a `default` (`#1C1C1C`) border. A checked row is marked by a `brand` border (not a fill change — the background still matches the surface).
...
A vertical stack matching the surface (`#EAE5DB`) with a `default` (`#1C1C1C`) border around the group: tight 2px corners at **`radius-xxl`**, with 2px `default` dividers between rows (functional row separators); each row padded `spacing-4`.
...
A full-width selectable card: padding `spacing-4`, background matching the surface (`#EAE5DB`), `default` (`#1C1C1C`) border, hover deepens the border toward `default-strong`, selected takes a `brand` border. The background never changes — only the border. The checkbox sits at the inline start; the content block holds a title + description.
